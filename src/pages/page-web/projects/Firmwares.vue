...
const columns = [
  { title: '型号', dataIndex: 'device_name', width: 120, ellipsis: true, className: 'titleStyle', slots: { customRender: 'device_name' } },
  ...
  { title: '创建者', dataIndex: 'username', width: 100, className: 'titleStyle' },
...
const rules = {
  ...
  device_name: [{ required: true, message: '请选择此固件所属的设备型号.' }]
}
interface FileItem {
  uid: string;
  name?: string;
...
const uploadFile = async () => {
  ...
  formRef.value.validate().then(async () => {
    ...
    uploading = (file.name ?? '上传中') + timestamp
    notification.open({
      key: uploading,
      message: `上传中  ${moment().format()}`,
      description: `[${file.name}] 正在上传中... `,
      duration: null
    })
    importFirmareFile(workspaceId, fileData).then((res) => {
      if (res.code === 0) {
        notification.success({
          message: `上传完成  ${moment().format()}`,
          description: `[${file.name}] 文件上传成功. 持续时间: ${moment.duration(new Date().getTime() - timestamp).asSeconds()} 秒`,
          duration: null
        })
        getAllFirmwares(pageParam)
      } else {
        notification.error({
          message: `上传失败 [${file.name}]. 请检查后重试.`,
