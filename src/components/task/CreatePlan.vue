  ...
  <div class="create-plan-wrapper">
    ...
    <div class="content">
      <a-form ref="valueRef" layout="horizontal" :hideRequiredMark="true" :rules="rules" :model="planBody" labelAlign="left">
        ...
        <!-- 航线 -->
        <a-form-item label="航线" :wrapperCol="{offset: 7}" name="file_id">
          <router-link
            :to="{name: 'select-plan'}"
            @click="selectRoute"
          >
        ...
        <a-form-item v-if="planBody.dock_sn" style="margin-top: -15px;">
          <div class="panel" style="padding-top: 5px;">
            ...
            <div class="ml10 mt5" style="color: hsla(0,0%,100%,0.65);">
              <span><RocketOutlined /></span>
              <span class="ml5">{{ dock.children?.nickname ?? '无无人机' }}</span>
            </div>
          </div>
        </a-form-item>
        <!-- 任务类型 -->
        <a-form-item label="计划定时器" class="plan-timer-form-item">
        ...
        <template v-if="planBody.task_type === TaskType.Condition">
          <!-- 电池电量 -->
          <a-form-item label="当电池电量达到以下百分比时启动任务" :labelCol="{span: 23}" name="min_battery_capacity">
          ...
          <!-- 存储容量 -->
          <a-form-item label="当存储容量达到以下数值时启动任务 (MB)" :labelCol="{span: 23}" name="storage_capacity">
            <a-input-number v-model:value="planBody.min_storage_capacity" class="width-100">
            </a-input-number>
          </a-form-item>
        </template>
        <!-- 返航高度 -->
        <a-form-item label="返航高度 (相对基站) (m)" :labelCol="{span: 23}" name="rth_altitude">
          <a-input-number v-model:value="planBody.rth_altitude" :min="20" :max="1500" class="width-100" required>
          </a-input-number>
        </a-form-item>
        <!-- 失控动作 -->
        <a-form-item label="失控动作" :labelCol="{span: 23}" name="out_of_control_action">
          <div style="white-space: nowrap;">
